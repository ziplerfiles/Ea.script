...
local ThemeManager = loadstring(game:HttpGet('https://raw.githubusercontent.com/Apollo153/rbxlmk/main/ThemeManger.lua'))()
local skybox_assets = loadstring(game:HttpGet("https://raw.githubusercontent.com/BigHacker123/skybox/main/.lua", true))()
--
local Build = "Paste";local Color = "#ffffff";local Ver = "v1.9b"
if Build == "Paste" then Color = '#ffffff' Ver = "v1.1a" end
local Window = Library:CreateWindow({ Size = UDim2.fromOffset(600, 720),Title = "<font color=\"#ffffff\">Oxide</font> 			".."<font color=\""..Color.."\">"..Build.."</font>".."                                         ["..Ver.."]",Center = true,AutoShow = true})
...
--// Notifications
Library:Notify(("Wussup thanks for loading this script bbg - "..game.Players.LocalPlayer.Name.." 🚬🅾🅿🅿"), 6)
Library:Notify(("Status: 🔥 - 40 "), 6)
...
SilentTab:AddToggle('Snapcircles',{Text='snapcircles',Default=false}):AddColorPicker('SnapcirclesColor',{Default=Color3.fromRGB(0,0,0),Title='Color'}):OnChanged(function(Value)
...
--
PlayerVisualTab:AddToggle('12t2ta22', {Text = 'onscreen',Default = false,Tooltip = 'Onscreen Information',}):AddColorPicker('OnscreenColor', {Default = Color3.fromRGB(0, 0, 0),Title = 'Color',}):OnChanged(function(ONSCREENVALUE)
...
PlayerVisualTab:AddToggle('Sleeping',{Text='names',Default=false}):AddColorPicker('SleepingColor',{Default=Color3.fromRGB(255, 255, 255),Title='Color'})
...
PlayerVisualTab:AddToggle('',{Text='full boxes',Default=false}):AddColorPicker('BoxesColor',{Default=Color3.fromRGB(255, 255, 255),Title='Color'}):AddColorPicker('BoxesFilledColor',{Default=Color3.fromRGB(0, 0, 0),Title='Color'}):OnChanged(function(Value)
...
PlayerVisualTab:AddToggle('Tool',{Text='weapon',Default=false}):AddColorPicker('ToolColor',{Default=Color3.fromRGB(255, 255, 255),Title='Color'})
...
PlayerVisualTab:AddToggle('HeadCircles',{Text='head circles',Default=false}):AddColorPicker('HeadCirclesColor',{Default=Color3.fromRGB(255, 255, 255),Title='Color'})
...
PlayerVisualTab:AddToggle('Tracer',{Text='tracer',Default=false}):AddColorPicker('TracerColor',{Default=Color3.fromRGB(255, 255, 255),Title='Color'})
...
do
CrosshairXTab:AddToggle('zCrosshairX_Toggle', {Text = 'enabled', Default = false}):AddColorPicker("eCrosshairX_Color", {Default = Color3.fromRGB(0, 0, 0)}):OnChanged(function()
...
inventoryviewerTab:AddToggle('InventoryViewerToggle',{Text='inv viewer',Default=false}):AddColorPicker('inventoryviewercolor',{Default=Color3.fromRGB(255,255,255),Title='Color'})
...
--
local velocityBarTabBox = Tabs.VisualsTab:AddRightTabbox('features currently down')
local velocityBarTab = velocityBarTabBox:AddTab('features currently down')
...
velocityBarTab:AddToggle("", {Text = "disabled",Default = false,}):OnChanged(function(state)
airTimeToggle = state
end)
...
XrayTab:AddToggle('Xray', {Text = 'enabled', Default = false, Tooltip}):AddKeyPicker('XRayKey', {Default = 'T', SyncToggleState = true, Mode = 'Toggle', Text = 'X-Ray', NoUI = true}):OnChanged(function(value)
XRayEnabled = value
if XRayEnabled then
...
ComExtraTab:AddLabel('deposit all'):AddKeyPicker('gazkb', { Default = 'Z', SyncToggleState = false, Mode = 'Toggle', Text = 'deposit all', NoUI = true })
...
MenuGroup:AddLabel(' Made by:<font color="#ffffff"> oxidepaid</font> 👑', true)
...
Library:SetWatermark('Oxide - FPS: ' .. fps .. ' | Ping: ' .. ping .. ' | [trident survival]')
...
SaveManager:SetFolder('x/folder')
SaveManager:BuildConfigSection(Tabs.UISettings)
SaveManager:IgnoreThemeSettings()
ThemeManager:SetLibrary(Library)
ThemeManager:SetFolder('x/themes')
ThemeManager:ApplyToTab(Tabs.UISettings)

--// Script Loaded
local Time = (string.format("%."..tostring(Decimals).."f", os.clock() - Clock))
Library:Notify(("Loaded Script (Ox) - "..tostring(Time).."s"), 15)
...
VelocityText.Color = Color3.fromRGB(0, 0, 0)
...
while true do
	VelocityText.Text = "oxide.exe | Insert to Open/Close\n" .. getTimeOfDay()
	wait(1)
end


-- Fake
warn("40 minute anti-cheat timer")
wait(3)
warn("loading bypasses {300/430}")
wait(2)
warn("Loaded all bypasses {999/430}")
wait(2400) 
warn("{systemalert*}! bans will now be issued around in 10-30 minutes, if you dont want this to continue to happen please join banland! ")
